...
.DESCRIPTION
    ...
    Путь БЕЗ Docker — для машины разработчика: поднимает продукт на локальном
    PostgreSQL, без образов и контейнеров. Рабочая установка, наоборот, живёт в
    Docker (docker-compose.yml, РАЗВЁРТЫВАНИЕ.md) — этот скрипт её не заменяет.
...
if (-not $SkipSeed) {
    # Сид досоздаёт недостающее; справочники, которые ведутся в приложении,
    # на живой базе не трогает. Его проверки здесь только предупреждают:
    # ставку ФОТ и веса труб проверяет smoke ниже, строгие сверки — в CI.
