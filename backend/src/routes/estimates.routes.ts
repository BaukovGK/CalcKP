...
estimatesRouter.patch('/:id/survey', requireRole('ADMIN', 'MANAGER', 'ENGINEER'), validate(surveyPatchSchema), async (req, res: Response, next: NextFunction) => {
  try {
    ...
    // APPROVED замораживает расчёт (Механика §10). Такой статус есть только у
    // расчётов, утверждённых до отмены согласования (решение 2026-07-20):
    // перехода в APPROVED больше нет, а обратного — APPROVED → CALC, «Новой
    // версии» — не было и прежде, поэтому ответ его не обещает.
    ...
    if (estimate.status === 'APPROVED') {
      res.status(422).json({
        message: 'Расчёт утверждён и заморожен: утверждённые расчёты не правятся',
