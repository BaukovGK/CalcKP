...
/**
 * SN в реальных ОЛ — проверка правила завода (utils/ring-stiffness.ts) на
 * тех же семи листах. Поле SN листа (E22) — расчётная ступень, а не
 * обозначение МВК: при «По ТТ МВК: Да» в нём стоят 5000 и 10000, никогда
 * 8000 и 12000.
 *
 * Прежнее правило сервера («глубина патрубка + 2 м» против 7 м, МВК внутри
 * жёсткости) не совпадало ни с одним из шести листов с МВК: оно давало 8000 и
 * 12000. Правило опросного листа (Нподз против 7000 мм, МВК только в
 * обозначении) совпадает с шестью из семи.
 *
 * Седьмой — Ол №3429 (КНС 750 м³/ч, корпус нестандартной формы): при Нподз
 * 8700 мм в листе SN 5000, а правило даёт 10000. Подбор предварительный,
 * окончательную жёсткость подтверждает расчёт прочности, — поэтому это
 * исключение, а не довод против правила.
 */
describe('SN в реальных ОЛ — правило завода', () => {
  it.each(ALL_EXAMPLES.filter((ex) => ex !== OL3429_KNS_750))('$file', (ex) => {
    const sn = ringStiffnessPa(ex.realNpodzMm)
    expect(sn).toBe(ex.input.ringStiffnessPa)
    // Обозначение по ТТ МВК — только в марку трубы.
    expect(ringStiffnessDesignation(sn, { mvk: ex.input.mvkRequired })).toBe(
      ex.input.mvkRequired ? (sn === 5000 ? 8000 : 12000) : sn,
    )
  })

  it(`${OL3429_KNS_750.file}: исключение — в листе 5000 при Нподз 8700 мм`, () => {
    expect(OL3429_KNS_750.input.ringStiffnessPa).toBe(5000)
    expect(ringStiffnessPa(OL3429_KNS_750.realNpodzMm)).toBe(10000)
  })
})
...
describe('roundUpTo', () => {
  ...
  // Как в Excel и во фронтенде: иначе на пороге жёсткости 7000 мм сервер и
  // опросный лист дали бы разный SN.
