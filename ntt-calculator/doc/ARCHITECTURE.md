...
Сервер смету не считает — с одним исключением, и оно на выпуске КП
(`POST /api/estimates/:id/kp` в `routes/estimates.routes.ts`). Там он
проверяет дерево: строки без цены (`rowsWithoutPrice` в
`backend/src/utils/estimate-tree.ts`), отрицательные строки
(`rowsWithNegativeSum`, 422 `NEGATIVE_ROWS`), ставки экономики не из прайса
(`treeRateFallbacks`, 422 `RATES_NOT_IN_PRICE`) — и САМ ИТОГ документа
(`verifyKpTotal` в `backend/src/utils/estimate-economics.ts`, 422
`TOTAL_MISMATCH`): цифра приходит с клиента, и печатать непроверенную
нельзя (решение Р7, План_устранения 3.5). Это вторая реализация
экономического пайплайна, и она намеренно узкая — разбора выражений в ней
нет, количество берётся посчитанным (`qtyResolved`), — а разойтись с
движком фронта ей не дают общие примеры `kp-total.vectors.json`: их
...
верным, чем два. Исключение выше — цена: её нельзя печатать заказчику со
слов клиента, поэтому экономический хвост (часы, ПЗР, ацетон, СИЗ,
накладные, себестоимость, цена продажи) продублирован на сервере под
общими примерами. Состав изделия — формулы шаблонов — остаётся в одном
месте.
...
в `composables/usePipeOverride.ts`, решение Р9), а при ТТ МВК показывает их
обозначениями 8000 и 12000 (`snManualOptions`); выбранное инженером
