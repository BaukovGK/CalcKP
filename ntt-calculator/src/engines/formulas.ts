 ...
 * ```
 *
 * Мс — масса формованных слоёв на стыке из справочника (f(Dу), см.
 * `MaterializeContext.jointLayerMassOf`). Косой стык длиннее прямого в
 * 1/0,707 = √2 раза (разрез под 45°), центральный считается за половину;
 * ×2 — на два днища.
 *
 * Контроль: Мс(DN3000) = 112 кг → 428,83 кг, как в эталоне.
 */
export function bottomJointLaminationKg(jointLayerMassKg: number): number {
  return (jointLayerMassKg / 0.707 + jointLayerMassKg / 2) * 2
}
