 ...
 *
 * **Закрыто ответом завода (2026-09-09).** 0,4 — класс САМОГО ИЗДЕЛИЯ: все
 * такие ёмкости безнапорные, и 0,4 у них принимается независимо от того, из
 * трубы 0,1, 0,6 или 1,0 они изготовлены. А У ТРУБ классы 0,1 и 0,4
 * идентичны 0,6 — это одна и та же труба, значит и вес погонного метра тот
 * же. Поэтому обе они разрешаются в 0,6, и DN1300 глубже 5 м находит вес
 * вместо «красной» строки.
 *
 * ⚠️ Это касается только ВЕСА. Ламинация считается по нормативу 0,4 — см.
 * `JOINT_LAYER_PN` в `stores/calcTree.ts`: справочник Мс задан в атмосферах,
 * и его строка «PN 4» — это те же 0,4 МПа. Две величины расходятся намеренно.
 *
...
export function pnForWeightLookup(pnSurvey: number, dn: number, sn: number): number {
  ...
  // Ветки эталона «→ 0,4» здесь нет намеренно: у ТРУБ классы 0,1 и 0,4
  // идентичны 0,6, и следующая строка накрывает оба. Возвращать 0,4, как
  // делает лист, значит искать несуществующий ключ.
