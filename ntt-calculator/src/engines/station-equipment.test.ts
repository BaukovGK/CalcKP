...
describe('шаблон КНС против настоящего прайса', () => {
  ...
  // Договорные позиции — те, что подбираются под проект, и трубы своего
  // производства: у них цены в прайсе нет и не должно быть. Любая другая
  // «красная» строка — опечатка в имени.
  it('«красные» только договорные позиции: трубы, насос, муфта, шкаф, задвижка на подводящем', () => {
    const red = flattenRows(materializeKns(ctx, REF))
      .filter((r) => r.priceCatalog == null && r.priceManual == null)
      .map((r) => r.name)
    expect(red).toEqual([
      'Труба СК/НПС-К 3000-0,1-12000',
      // Гильза подводящего DN400 и фланцевый патрубок под задвижку — отрезки
      // трубы Ø500 (лист, строки 27 и 33).
      'Труба СК/НПС-К 500-0,1-2500',
      'Труба СК/НПС-К 500-0,1-2500',
