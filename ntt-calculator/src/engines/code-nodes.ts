...
export const BUILTIN_NODES: readonly BuiltinNode[] = [
  ...
  kns({ ref: 'kns.inletFlange', codes: ['A6'], title: 'Фланцевый патрубок под задвижку на подводящем', reads: 'DN и число подводящих', build: buildKnsInletFlange }),
  ...
  kns({
    ...
    build: (ctx, s) => [
      buildBasket(ctx, { device: 'KNS', dn: s.dn, trayDepthMm: s.inletTrayDepthMm, enabled: knsBasketOn(s), withGrinder: Boolean(s.hasGrinder) }),
    ],
  }),
  ...
  kns({ ref: 'kns.valves', codes: ['C4'], title: 'Узел запорной арматуры', reads: 'патрубки, насосы, ручные количества ОЛ', build: buildKnsValves }),
