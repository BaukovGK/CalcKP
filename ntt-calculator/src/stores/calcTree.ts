...
/**
 * Давление, при котором берётся Мс из справочника f(Dу, PN), атм.
 *
 ...
 *
 * Это НЕ упрощение и не дефект: завод подтвердил (2026-09-09), что ламинация
 * считается по нормативу 0,4. Справочник Мс задан в атмосферах, поэтому его
 * «PN 4» — те же 0,4 МПа, что и класс изделия. Остальные строки таблицы к
 * ламинации отношения не имеют.
 *
 * Не путать с поиском ВЕСА трубы: там 0,1 и 0,4 разрешаются в 0,6, потому что
 * это одна и та же труба (`pnForWeightLookup`, engines/survey-kns.ts).
 */
