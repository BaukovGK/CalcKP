...
const liveValues = computed(() => {
  ...
  return [
    { k: 'Рабочая часть', v: `${form.value.depthMm} мм`, hint: { title: 'Рабочая часть', text: 'Глубина H из опросного листа.' } },
    {
      ...
      hint: { title: 'Горловина', text: 'Высота горловины вместе с возвышением — длина её трубы и прибавка к полной глубине.', formula: 'h горловины + возвышение', source: 'Эталон N5' },
    },
    {
      k: 'SN',
      v: String(g.sn ?? '—'),
...
const blocks = computed(() => [
  ...
  { t: 'Перекрытие и люк', on: true },
  { t: 'Вентиляционный стояк', on: true },
  { t: 'Крепёж', on: true },
  // Запорная арматура в блоки не входит: в листе колодца её строки
  // вписываются руками, раздел «Оборудование» заполняется в расчёте.
])
