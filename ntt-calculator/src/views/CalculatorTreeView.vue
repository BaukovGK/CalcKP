...
const KP_HINT: Hint = {
  ...
  text: [
    'Печатная форма КП в Word и PDF и новая версия расчёта. Пока есть строки без цены, КП не выпускается.',
    'Перед выпуском расчёт сохраняется, а сервер пересчитывает итог по сохранённому дереву и сверяет с показанным здесь: в документ заказчику идёт только проверенная цифра.',
  ],
}
