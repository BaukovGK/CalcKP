...
function bucketReason(row: CalcRowNode): string {
  ...
  if (row.bucket === 'Труба, муфта') return 'В итогах — «Труба, муфта»: трубы и муфты своего производства выделены отдельно.'
...
export function calcRowHint(row: CalcRowNode, res: RowResult, ctx: RowHintContext): Hint {
  ...
  if (ctx.conflict && row.renamedFrom != null) {
    tone = 'warn'
    text.push(
      `Наименование сменилось вместе с параметром опросного листа: было «${row.renamedFrom}». ` +
        'Ручные правки прежней строки перенесены сюда — проверьте их: «Оставить моё» сохранит, ' +
        '«Принять новое» вернёт расчётные количество и цену.',
    )
  } else if (ctx.conflict) {
    tone = 'warn'
    text.push(
...
export const BUCKET_HINTS: Record<CostBucket, Hint> = {
  ...
  'Труба, муфта': {
    title: 'Труба, муфта',
    text: 'Стеклопластиковые трубы своего производства: корпус и его сегменты, труба шахты или горловины, отрезки трубы под гильзы — и муфты. Цена трубы договорная — из поля опросного листа.',
...
export const FILTER_HINTS = {
  ...
  conflict: {
    title: 'Конфликты',
    text: [
      'Опросный лист изменил расчётное количество у строки, где стоит ручное, или сменил наименование строки с ручными правками.',
      'Решите в строке: оставить своё или принять новое.',
    ],
    tone: 'warn',
  },
  override: {
